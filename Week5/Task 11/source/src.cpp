...
MoneyFlow operator+(MoneyFlow const & a, MoneyFlow const & b) {
  return {a.earning + b.earning, a.spending + b.spending};
}
...
public:
  ...
  BulkLinearUpdater(const BulkMoneyRemover& rem)
          : rem_(rem)
  {}
  ...
  void CombineWith(const BulkLinearUpdater& other) {
    tax_.multiplier *= other.tax_.ComputeFactor();
    add_.delta = add_.delta * other.tax_.ComputeFactor() + other.add_.delta;
    rem_.delta += other.rem_.delta;
  }

  MoneyFlow Collapse(MoneyFlow const & origin, IndexSegment segment) const {
    MoneyFlow result;
    result.earning = origin.earning * tax_.ComputeFactor() + add_.delta * segment.length();
    result.spending = origin.spending + rem_.delta * segment.length();
    return result;
  }
...
class BudgetManager : public SummingSegmentTree<MoneyFlow, BulkLinearUpdater> {
public:
...
struct SpendRequest : ModifyRequest {
  ...
  void Process(BudgetManager& manager) const override {
    ...
    manager.AddBulkOperation(date_segment, BulkMoneyRemover{daily_spend});
  }
