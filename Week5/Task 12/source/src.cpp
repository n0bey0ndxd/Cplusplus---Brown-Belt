...
void PushRem(size_t v, size_t l, size_t r) {
  for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
    if (w < VERTEX_COUNT) {
...
double ComputeSumRem(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
  if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
    return 0;
  }
  PushRem(v, l, r);
  if (ql <= l && r <= qr) {
    return tree_values_spend[v];
  }
  return ComputeSumRem(v * 2, l, (l + r) / 2, ql, qr)
         + ComputeSumRem(v * 2 + 1, (l + r) / 2, r, ql, qr);
}

double Compute(size_t v, size_t l, size_t r, size_t ql, size_t qr) {
  return ComputeSum(v, l, r, ql, qr) - ComputeSumRem(v, l, r, ql, qr);
}
...
void AddRem(size_t v, size_t l, size_t r, size_t ql, size_t qr, double value) {
  if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
    return;
  }
  PushRem(v, l, r);
  ...
  AddRem(v * 2, l, (l + r) / 2, ql, qr, value);
  AddRem(v * 2 + 1, (l + r) / 2, r, ql, qr, value);
  tree_values_spend[v] =
          (v * 2 < VERTEX_COUNT ? tree_values_spend[v * 2] : 0)
          + (v * 2 + 1 < VERTEX_COUNT ? tree_values_spend[v * 2 + 1] : 0);
}

void Multiply(size_t v, size_t l, size_t r, double factor, size_t ql, size_t qr) {
  if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
    return;
  }
  Push(v, l, r);
  ...
  Multiply(v * 2, l, (l + r) / 2, factor, ql, qr);
  Multiply(v * 2 + 1, (l + r) / 2, r, factor, ql, qr);
  tree_values[v] =
          (v * 2 < VERTEX_COUNT ? tree_values[v * 2] : 0)
          + (v * 2 + 1 < VERTEX_COUNT ? tree_values[v * 2 + 1] : 0);
}
...
int main() {
  ...
  for (int i = 0; i < q; ++i) {
    ...
    if (query_type == "ComputeIncome") {
      cout << Compute(1, 0, DAY_COUNT_P2, idx_from, idx_to) << endl;
    } else if (query_type == "PayTax") {
      int tax_percentage;
      cin >> tax_percentage;
      double factor = 1.0 - tax_percentage / 100.0;
      Multiply(1, 0, DAY_COUNT_P2, factor, idx_from, idx_to);
      ...
      double value;
      cin >> value;
      AddRem(1, 0, DAY_COUNT_P2, idx_from, idx_to, value / (idx_to - idx_from));
    }
  }
