...
ostream& operator << (ostream& output, const HttpResponse& resp) {
    output << "HTTP/1.1 " << resp.GetCodeString() << "\n";
    ...
    if (!resp.content.empty()) {
        output << "Content-Length: " << resp.content.length() << "\n\n";
    ...
    HttpResponse ServeRequest(const HttpRequest& req) {
        ...
        if (req.method == "POST") {
            ...
            if (req.path == "/user_comments") {
                ...
                resp.SetCode(HttpCode::Ok).SetContent("What's the answer for The Ultimate Question of Life, the Universe, and Everything?");
            }
        }
