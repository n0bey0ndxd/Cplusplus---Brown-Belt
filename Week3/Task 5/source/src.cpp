...
class Polynomial {
    ...
    public:
        ...
        T & operator= (T const & other) {
            if (other == 0 && poly.coeffs_.size() <= degree) {
                return t;
            }

            while (poly.coeffs_.size() <= degree) {
                poly.coeffs_.push_back(0);
            }

            poly.coeffs_[degree] = other;
            poly.Shrink();
            return poly.coeffs_.size() <= degree ? t : poly.coeffs_.at(degree);
        }
    };
