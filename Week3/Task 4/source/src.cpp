...
public:
    ...
    struct ReadAccess {
        ReadAccess(const V& ref_v, mutex& mx)
            :
            ref_to_mutex(mx),
            ref_to_value(ref_v)
        {}
    ...
    explicit ConcurrentMap(size_t bucket_count) 
        :
        storage(vector<Bucket<K, V, Hash>>(bucket_count))
    {}

    WriteAccess operator[](const K& key) {
        size_t index = getBucket(key);
        storage[index].m.lock();
        return { storage[index].data[key], storage[index].m };
    }

    ReadAccess At(const K& key) const {
        size_t index = getBucket(key);
        storage[index].m.lock();
        return { storage[index].data.at(key), storage[index].m };
    }

    bool Has(const K& key) const {
        return static_cast<bool>(storage[getBucket(key)].data.count(key));
    }

    MapType BuildOrdinaryMap() const {
        MapType result;
        for (auto& bucket : storage) {
            lock_guard lg(bucket.m);
...
private:
    Hash hasher;
    vector<Bucket<K, V, Hash>> storage;

    size_t getBucket(const K& key) const {
        return hasher(key) % storage.size();
    }
};
