...
private:
  std::optional<BusInfo> CalculateInfoForBus(std::string_view bus_name) const;
...
Request ParseRequest(std::string_view request_str, bool isCreating) {
  ...
  if (type_str == BUS_STR) {
    ...
    return Request(isCreating ?
                  REQUEST_TYPE::CREATING_STOP : REQUEST_TYPE::STOP,
                  std::string(request_str));
  }
}
  ...
  for (auto const & request : requests) {
    switch (request.GetType()) {
      ...
      case REQUEST_TYPE::BUS: {
        auto bus = bus_model::ParseBus(request.GetRequestBody());
        std::optional<TransportBase::BusInfo> info = CalculateInfoForBus(bus->GetName());
        if (info.has_value()) {
      ...
      case REQUEST_TYPE::STOP:
      {
        ...
        std::string s = "Stop " + std::string(stop_name) + ":";

        auto it = m_stop_name_and_bus_names.find(stop_name);
        if (it != m_stop_name_and_bus_names.end() && m_stops.find(stop_name) != m_stops.end()) {
          auto const & buses_names = it->second;
          if (buses_names.size()) {
            s += " buses";
            for (auto bus_name : it->second) {
              s += " " + std::string(bus_name);
            }
          }
          else {
            s += " no buses";
        ...
        responses.emplace_back(std::move(s));
      }
    }
  }
...
std::optional<TransportBase::BusInfo> TransportBase::CalculateInfoForBus(std::string_view bus_name) const {
